...
- **看見差距是多少**：每一里都標出當選方與落後方相差幾票、要多少新票進場才能拉平 — 把「贏 / 輸」變成可看見的距離
...
- 按下「複製分享連結、點亮燈塔」→ 該里記一次 `share`
- 別人從帶 `?ref=share` 的分享連結進來 → 該里記一次 `view`
- 燈塔分數 = `share + view`，累積到 **10 次** → 里級燈塔點亮（Lv.1）
- 每多 10 次升一級，最高 Lv.10；拉遠看全區時，區內累積達 **50 次** 會顯示區級聚合燈塔
- 防刷分兩層：前端同一瀏覽器 30 分鐘內不重複送同一事件；Worker 在正式站以 IP hash 對同一里做約 24 小時鎖（`share` / `view` 共用）
- 計數讀取有 60 秒快取；成功分享後前端會先本地 +1，讓畫面立即更新
- 每日排程會讓每個里的 `share` 與 `view` 各 −1，歸零後移除；沒人持續分享的燈塔會慢慢矮回去
